...
class QLearner:
    def __init__(self, game, available_actions, discount_factor=0.999, learning_rate=1.0):
        self.game = game
        self.available_actions = available_actions
        dimensions = self.game.get_map_dimensions()
        self.q = [[[0.0 for action in self.available_actions] for x in range(dimensions[1])] for y in range(dimensions[0])]
    ...
    def get_q_list_at_pos(self):
        return self.q[game.get_y()][game.get_x()]
